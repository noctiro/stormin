...
use ratatui::{
    prelude::*,
    widgets::{Block, Borders, Gauge, Paragraph, Wrap, LineGauge, BarChart},
    symbols,
};
use sysinfo::System;
...
pub struct Stats {
    ...
    pub debug_logs: Vec<DebugInfo>,  // 新增的调试日志
}
...
pub fn draw_ui<B: Backend>(terminal: &mut Terminal<B>, stats: &Stats) -> std::io::Result<()> {
    terminal.draw(|f| {
        ...
        let debug_messages: Vec<Line> = stats.debug_logs.iter()
            .map(|log| {
        ...
        let title = format!(
            "Stormin Attack Dashboard {} [Press P:Pause R:Resume Q:Quit]",
        ...
        // 内存使用率图表
        let memory_ratio = stats.memory_usage as f64 / (stats.sys.total_memory() as f64);
        ...
        let success = Paragraph::new(vec![
            Line::from(vec![
                ...
                Span::styled(
                    format!(" ({:.1}s)", stats.last_success_time
        ...
        let failure = Paragraph::new(vec![
            Line::from(vec![
                ...
                Span::styled(
                    format!(" ({:.1}s)", stats.last_failure_time
                        .map(|t| t.elapsed().as_secs_f64())
                        .unwrap_or(0.0)
                    ),
                    Style::default().fg(Color::DarkGray)
        ...
        // 请求统计图表
        let requests_chart_data = stats.targets.iter().map(|t| {
        ...
        let barchart = BarChart::default()
            .block(Block::default().title("Requests by Target").borders(Borders::ALL))
        ...
        // 线程状态
        let thread_info: Vec<Line> = stats.threads.iter().map(|t| {
        ...
        // Target状态
        let targets_info: Vec<Line> = stats.targets.iter().flat_map(|t| {
            ...
            let last_success = t.last_success_time
                .map(|t| t.elapsed().as_secs_f64())
                .map(|s| format!("{:.1}s ago", s))
                .unwrap_or_else(|| "N/A".to_string());
                
            let last_failure = t.last_failure_time
                .map(|t| t.elapsed().as_secs_f64())
                .map(|s| format!("{:.1}s ago", s))
                .unwrap_or_else(|| "N/A".to_string());
...
pub fn try_copy_log_entry(stats: &Stats, row: u16) -> io::Result<bool> {
    ...
    if entry_index < stats.debug_logs.len() {
        if let Ok(mut ctx) = ClipboardContext::new() {
            let _ = ctx.set_contents(stats.debug_logs[entry_index].message.clone());
            return Ok(true);
        }
    }
    Ok(false)
